...
const Schedule = () => {
  const [selectedHostName, setSelectedHostName] = React.useState<string | null>(
    null
  );
  const [schools, setSchools] = React.useState<any[]>([]);
  const [selectedSchool, setSelectedSchool] =
    React.useState<SelectedSchool | null>(null);
  const [classes, setClasses] = React.useState<any[]>([]);
  const [selectedClass, setSelectedClass] =
    React.useState<SelectedClass | null>(null);
        ...
        >
          <Paper sx={{ padding: 2, marginRight: 1, marginTop: 2 }}>
            ...
            <Autocomplete
              ...
              sx={{ width: 300 }}
              renderInput={(params) => (
                <TextField {...params} label="Hostname" />
              )}
            />
          </Paper>
          <Stack
            ...
            <Paper sx={{ padding: 2, marginRight: 1, marginTop: 2 }}>
              ...
              <Autocomplete
                ...
                isOptionEqualToValue={(option, value) =>
                  option.unitGuid === value.unitGuid
                }
                onChange={(event, value: any) => handleSchoolChange(value)}
                sx={{ width: 300 }}
                renderInput={(params) => (
                  <TextField {...params} label="Skola" />
                )}
              />
            </Paper>
