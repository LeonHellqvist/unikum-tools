...
const Bookmarks = () => {
  ...
  const selectBookmark = (uuid: string, bgColor: string) => {
    const bookmark = bookmarks.find((bookmark: { uuid: string }) => bookmark.uuid === uuid);
    setColor(bgColor);
    ...
    <Container maxWidth="md" sx={{ marginTop: 2 }}>
      ...
      <Paper elevation={3} sx={{ width: '100%', height: '100%', paddingBottom: 2 }}>
        <Stack direction="column" spacing={1}>
          <BookmarkBar bookmarks={bookmarks} setBookmarks={setBookmarks} selectBookmark={selectBookmark} />
          <Box>
            <Grid container>
              ...
              <Grid item xs={4} sx={{ paddingLeft: 1, paddingRight: 2 }}>
                <Stack direction="column" spacing={1}>
                  <GlobalAlternatives settings={settings} setSettings={setSettings} />
                  <BackgroundColor
                    color={color}
                    setColor={setColor}
                    selectedBookmark={selectedBookmark}
